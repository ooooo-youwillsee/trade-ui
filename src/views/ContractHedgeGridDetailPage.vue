...
import ContractHedgeGridResults from '../components/ContractHedgeGridResults.vue';
...
const currentTitle = computed(() => selectedStrategy.value?.name || '对冲网格详情');
</script>
...
<template>
  <section class="mobile-page contract-detail-page">
    ...
    >
      <template #right>
        <van-button
          ...
          type="primary"
          round
          @click="router.push(`/contract/hedge-grid/${route.params.id}/edit`)"
        >
          编辑
        </van-button>
      </template>
    ...
    <ContractHedgeGridResults :active-input="activeInput" :result="result" />
  </section>
</template>
